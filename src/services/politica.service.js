...
class PoliticaService {
  async listar(query = {}) {
    ...
    if (query.categoria && query.categoria !== 'todas') {
      if (!CATEGORIAS_VALIDAS.includes(query.categoria)) {
        const error = new Error('Categoría de política inválida'); error.status = 400; throw error;
      }
  ...
  async obtenerPrincipal() {
    ...
    if (!principal) {
      const error = new Error('No hay un mensaje principal configurado'); error.status = 404; throw error;
    }
    return principal;
  }
  ...
  async obtenerPorId(id) {
    ...
    if (!politica) {
      const error = new Error('Política no encontrada'); error.status = 404; throw error;
    }
    return politica;
  }
  ...
  async crear(datos) {
    ...
    if (!titulo || !contenido) {
      const error = new Error('Los campos título y contenido son obligatorios'); error.status = 422; throw error;
    }
    if (categoria && !CATEGORIAS_VALIDAS.includes(categoria)) {
      const error = new Error('Categoría de política inválida'); error.status = 400; throw error;
    }
    if (datos.esPrincipal) await politicaRepository.desmarcarPrincipales();
  ...
  async actualizar(id, datos) {
    ...
    if (datos.esPrincipal) await politicaRepository.desmarcarPrincipales(id);
